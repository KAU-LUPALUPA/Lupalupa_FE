...
| 내 친구 코드 조회 | 친구 | 로그인 사용자 | GET | `/friends/users/me/friend-code` | 없음 | 내 친구 코드를 조회한다. | `friendCode`, `displayFriendCode` | 친구 화면에서 쓰는 유저 조회성 API는 `/friends/**` 아래에 둔다. 화면에는 `displayFriendCode`, API 요청에는 `friendCode` 사용을 권장한다. |
